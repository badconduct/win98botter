...
test("gateway profile and secret-file token preserve the existing tool-result loop", async (t) => {
  ...
  const previous = Object.fromEntries(["BOT_API_KEY_FILE", "BOT_AI_PROFILE"].map(k => [k,process.env[k]]));
  ...
  assert.throws(()=>new LLMClient("http://127.0.0.1/v1","fallback","default"),/nonempty token/);
  fs.unlinkSync(tokenFile);
  assert.throws(()=>new LLMClient("http://127.0.0.1/v1","fallback","default"),/ENOENT/);
});
